...
pub fn on_chunk_load_notify_lighting_worker(game: &mut Game, pos: ChunkPosition) {
    let handle = game
        .chunk_map
        .chunk_handle_at(pos)
    ...
    game.lighting_worker_handle
        .tx
        .send(Request::LoadChunk { pos, handle })
        .expect("failed to notify lighting worker of chunk load");
}

pub fn on_chunk_unload_notify_lighting_worker(game: &mut Game, pos: ChunkPosition) {
    game.lighting_worker_handle
        .tx
        .send(Request::UnloadChunk { pos })
...
pub fn start_worker() -> LightingWorkerHandle {
...
fn run_worker(rx: crossbeam::Receiver<Request>, shutdown_tx: crossbeam::Sender<()>) {
    ...
    info!("Lighting worker shutting down");
