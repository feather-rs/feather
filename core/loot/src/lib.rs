...
static STORE: Lazy<AHashMap<InlinableString, LootTable>> = Lazy::new(|| {
    ...
    serde_cbor::from_slice::<LootTableSet>(BYTES)
...
fn apply_functions<'a>(
    ...
    for function in functions {
        match &function.kind {
            FunctionKind::LimitCount { count } => {
                let count = count.sample(rng);
                item.amount = item.amount.min(count as u8);
            }
...
mod tests {
    ...
    #[test]
    fn sample_stone() {
        let table = loot_table("blocks/stone").expect("missing loot table for stone block");

        let mut rng = StepRng::new(0, 1);

        let items = table.sample(&mut rng).unwrap();

        assert_eq!(items.as_slice(), &[ItemStack::new(Item::Stone, 1)]);
    }
}
