...
use quill_common::components::Hunger;
        ...
        match hunger.food {
            20 if hunger.saturation > 0 => {
                if game.tick_count % 10 == 0 {
                    events.push((player, UpdateHealthEvent::new(HealthEventType::Heal(1))));
                }
            }
            18..=20 => {
            ...
            0 => {
                if game.tick_count % 80 == 0 {
                    events.push((player, UpdateHealthEvent::new(HealthEventType::Harm(1))));
                }
            }
            _ => {}
        }
