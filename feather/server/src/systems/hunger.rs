...
use quill_common::{
    components::Hunger,
    events::{entity_damage_event_kind, entity_regen_event_kind},
};
        ...
        match hunger.food {
            20 if hunger.saturation > 0 => {
                if game.tick_count % 10 == 0 {
                    eating_events.push((player, entity_regen_event_kind::Eating));
                }
            }
            18..=20 => {
            ...
            0 => {
                if game.tick_count % 80 == 0 {
                    starving_events.push((player, entity_damage_event_kind::Starvation));
                }
            }
            _ => {}
        }
