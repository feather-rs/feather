...
#[derive(Default)]
pub struct WaitingChunks(AHashMap<ChunkPosition, Vec<EntityId>>);

impl WaitingChunks {
    pub fn drain_players_waiting_for(&mut self, chunk: ChunkPosition) -> Vec<EntityId> {
        self.0.remove(&chunk).unwrap_or_default()
    }

    pub fn insert(&mut self, player: EntityId, chunk: ChunkPosition) {
