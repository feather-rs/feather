...
impl EntitySelector {
    ...
    pub fn entities_to_string(&self, ctx: &CommandCtx, add_player: bool) -> String {
        if self.entities.is_empty() {
            ...
            if let Ok(name) = ctx.ecs.get::<Name>(*self.entities.first().unwrap()) {
                if add_player {
                    if ctx
                        .ecs
                        .get::<Player>(*self.entities.first().unwrap())
                        .is_ok()
                    {
                        format!("player {}", *name)
                    } else {
...
fn find_selected_entities(
    ...
    Ok(match input {
        "@p" => {
            ...
            let pos = ctx
                .ecs
                .get::<Position>(ctx.sender)
                .map(|r| *r)
                .unwrap_or_default();

            nearest_player_to(&ctx.ecs, pos).into_iter().collect()
        }
        "@r" => {
            // Random player
            random_player(&ctx.ecs).into_iter().collect()
        }
        "@a" => {
            // Every player
            ctx.ecs.query::<&Player>().iter().map(|(e, _)| e).collect()
        }
        "@e" => {
            // Every entity
            ctx.ecs
        ...
        player_name => smallvec![find_player_by_name(&ctx.ecs, player_name)
...
fn find_player_by_name(ecs: &Ecs, name: &str) -> Option<Entity> {
    ecs.query::<&Name>()
        .iter()
        .find(|(_, n)| &***n == name)
...
fn nearest_player_to(ecs: &Ecs, pos: Position) -> Option<Entity> {
    ecs.query::<(&Position, &Player)>()
        .iter()
        .min_by_key(|(_, (p, _))| pos.distance_squared_to(**p).floor() as u64)
        .map(|(entity, _)| entity)
}

fn random_player(ecs: &Ecs) -> Option<Entity> {
    let mut query = ecs.query::<&Player>();

    let count = query.iter().count();

    let index = rand::thread_rng().gen_range(0, count);

    query.iter().nth(index).map(|(e, _)| e)
}
